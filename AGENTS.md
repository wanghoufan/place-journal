...
- `docs/handoff/HANDOFF.md` §0 为现役收工快照（2026-09-04 深夜，Vercel 上线后）；`docs/acceptance-l2/`、`docs/qa/`、`docs/db/ENV1-实测记录丨2026-09-04.md` 为证据；数据库治理权威在 `alw丨数据库管理专家/项目审查丨habit_tracker/`。
- 全部查询走 `supabase.schema('habit_tracker')`；禁止业务表进 `public`、禁止整库回写、禁止 `service_role` 进前端/Git。

**当前状态与下一步**
- 已完成（2026-09-04）：M0–M6 + 主题系统 + 分享长图/卡片图 + 标签真删 + Find 分离；S1（`20260903141849`）/S2/L2_PASS/TAG_FIX_PASS/QA_V02_PASS；ENV-1 已关闭（0003 上线 + 9 项实测 + 管理员复审通过）；ASR 真转写 + AI（`deepseek-v4-flash`，12s 降级保留）全通；Docker 8081 已上线（healthy）；QA_FAIL/VA_FAIL 经甄别真问题 2 个已修（Find 取消选中 `b819931`、8081 OAuth 白名单）；Record 三项修复 + OBS-1（Stars span）已修并推送（HEAD=origin/master=`bae604f`，2026-09-04 验证一致）；Vercel 生产 `https://place-journal-xi.vercel.app` 已上线+验收（transcribe 真音频 PASS，手机 HTTPS 录音用户实测成功；Vercel QA 回归 PASS_WITH_BLOCKED 见 `docs/qa/QA回归报告丨Vercel上线丨2026-09-04.md`）。RQA-V 真机修复（revoke 按 slug、删记录级联、OBS-V2 上游超时）已改未提交，见 HANDOFF §0。
- 待办：高德 Key 联调（Vercel/8081 两端补变量）；历史验收标签清理待示下；单设备定位（L3 取消）；Tailscale 已被 Vercel 取代。
